...
impl Response {
    pub fn new((cont, valid): (bool, bool)) -> Self {
        Self { cont, valid }
    }
}
...
fn do_verify(client_id: &str, res: &BigNum) -> Result<Response, DbTxErr> {
    ...
    let tx_res: TransactionResult<(bool, bool), DbTxErr> =
        DB.lock().unwrap().transaction(|tx_db| {
